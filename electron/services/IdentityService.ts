...
export class IdentityService {
   ...
   * Encodes the Ed25519 private key using BIP39 wordlist.
  ...
  async exportSeedPhrase(): Promise<string[]> {
    ...
    const privateKeyBytes = Buffer.from(stored.secretKey, 'hex')
    // Compute checksum: first byte of SHA-256 of private key
    const hash = crypto.createHash('sha256').update(privateKeyBytes).digest()
    const checksumByte = hash[0]
    
    // Combine 32 bytes (256 bits) + 8 bits checksum = 264 bits = 24 × 11 bits
    const bits: boolean[] = []
    ...
    // Convert 24 groups of 11 bits to word indices
    const words: string[] = []
    for (let i = 0; i < 24; i++) {
      let index = 0
      for (let j = 0; j < 11; j++) {
  ...
  async importSeedPhrase(words: string[]): Promise<IdentityResult> {
    ...
    // Reconstruct Ed25519 key pair from private key bytes
...
// BIP39 English wordlist (2048 words) for seed phrase encoding
