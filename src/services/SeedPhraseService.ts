/**
 * SeedPhraseService — Generates and restores identity from a 24-word seed phrase.
 * Similar to Keet's 24-word seed phrase for identity backup/restore.
 * 
 * Uses a simplified word list (256 words) where each word represents 1 byte.
 * The seed phrase encodes the first 24 bytes of the secret key.
 */

// 256-word list for encoding bytes (each word = 1 byte)
// Based on EFF's wordlist but simplified for our use case
const WORD_LIST = [
  // A
...
/**
 * Generate a 24-word seed phrase from a secret key.
 * Takes the first 24 bytes of the secret key and maps each to a word.
 */
export function secretKeyToSeedPhrase(secretKeyHex: string): string {
  // Secret key is 64 bytes (128 hex chars), we use first 24 bytes
  const bytes = hexToBytes(secretKeyHex.slice(0, 48))
  
  if (bytes.length < 24) {
    throw new Error('Secret key too short for seed phrase')
  }
  
  const words: string[] = []
  for (let i = 0; i < 24; i++) {
    words.push(WORD_LIST[bytes[i]])
  }
...
/**
 * Restore a secret key from a 24-word seed phrase.
 * Returns the first 24 bytes of the secret key.
 * Note: This is used to derive the full key pair.
 */
export function seedPhraseToSecretKeyBytes(seedPhrase: string): Uint8Array {
  const words = seedPhrase.toLowerCase().trim().split(/\s+/)
  
  if (words.length !== 24) {
    throw new Error(`Seed phrase must be 24 words, got ${words.length}`)
  }
  ...
  for (let i = 0; i < 24; i++) {
    ...
    if (index === undefined) {
      throw new Error(`Unknown word in seed phrase: "${words[i]}"`)
    }
    bytes[i] = index
  }
...
/**
 * Get the word list for display/autocomplete.
 */
