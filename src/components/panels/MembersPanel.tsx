...
const MemberGroup: React.FC<{
  ...
  // L’étiquette est lue par les lecteurs d’écran : un « Muted » en dur serait
  // resté anglais pour un utilisateur malvoyant, quelle que soit sa langue —
  // d’où le passage en corps de fonction pour pouvoir appeler `useTranslation`.
  const { t } = useTranslation()
  return (
    <div>
      ...
      <div className="space-y-1">
        {members.map((member) => (
          <div key={member.publicKey} className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-asgard-surface-alt transition-colors">
            ...
            {member.mutedBy && (
              <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" className="text-asgard-busy" aria-label={t('common.muted')}>
